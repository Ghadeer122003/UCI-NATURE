...
from server.scripts.config import APP_ENV, PIPELINE_DRIVE_CACHE_POLICY
from server.scripts.pipeline.make_output import generate_output_csvs
from server.scripts.pipeline.review_decisions import (
...
from server.scripts.pipeline.simple_outputs import (
    ANIMAL_RESULTS_CSV,
    FINAL_RESULTS_CSV,
    REVIEW_NEEDED_CSV,
...
from server.scripts.pipeline.validate_output import validate_csv, REQUIRED_COLUMNS
from server.auth.routes import get_google_auth_state, router as google_auth_router
from server.auth.routes_drive import (
...
from server.cancellation import CancellationToken, OperationCancelled
from server.services.drive_staging_service import stage_selected_drive_folder
from server.session_store import create_session_token, read_session, write_session
...
def validate_pipeline_prerequisites(
    ...
    *,
    ...
    from server.services.pipeline_service import resolve_pipeline_staging_dir
    ...
    try:
        with open(log_path, "w", encoding="utf-8", buffering=1) as log_file:
            with redirect_stdout(log_file), redirect_stderr(log_file):
                from server.services import PipelineRunConfig, run_pipeline_service
                from server.services.pipeline_service import resolve_pipeline_staging_dir
